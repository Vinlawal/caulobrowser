...
mod_expression_server <- function(id, gene_results, db_con) {
  shiny::moduleServer(id, function(input, output, session) {
      ...
      tryCatch(
        {
          ...
          if (length(backgrounds) > 0) {
            shiny::updateSelectInput(session, "genetic_background", choices = backgrounds)
          } else {
            shiny::updateSelectInput(
              session, "genetic_background",
    ...
    expression_data <- shiny::reactive({
      genes <- gene_results()
      shiny::req(nrow(genes) > 0, input$genetic_background, nchar(input$genetic_background) > 0)
      get_expression_data(db_con(), genes$gene_id, genetic_background = input$genetic_background)
    })
    ...
    experiment_meta <- shiny::reactive({
      ...
      unique(df[, c("experiment_id", "experiment_type", "display_label", "doi")])
    })
    ...
    output$expr_plots <- shiny::renderUI({
      ...
      type_order <- c("microarray", "rnaseq", "riboseq", "proteomics")
      ...
      groups <- lapply(ordered_types, function(dtype) {
        ...
        cards <- lapply(seq_len(nrow(exps)), function(i) {
          ...
          doi_link <- if (!is.null(doi_val) && !is.na(doi_val) && nzchar(doi_val)) {
            shiny::tags$a(
          ...
          shiny::div(
            ...
            plotly::plotlyOutput(ns(output_id), height = "300px")
          )
        })
      ...
      lapply(seq_len(nrow(meta)), function(i) {
        local({
          ...
          output[[output_id]] <- plotly::renderPlotly({
            ...
            plot_single_expression(sub_df, title = exp_label, color_palette = pal)
          })
