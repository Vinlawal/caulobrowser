...
CREATE TABLE timecourse_expression (
  ...
  DBI::dbExecute(con, "CREATE INDEX idx_de_exp      ON de_results(experiment_id);")
  DBI::dbExecute(con, "CREATE INDEX idx_de_gene     ON de_results(gene_id);")
  DBI::dbExecute(con, "CREATE INDEX idx_tc_gene     ON timecourse_expression(gene_id);")
  DBI::dbExecute(con, "CREATE INDEX idx_tc_exp      ON timecourse_expression(experiment_id);")
  DBI::dbExecute(con, "CREATE INDEX idx_tc_gene_exp ON timecourse_expression(gene_id, experiment_id);")
  DBI::dbExecute(con, "CREATE INDEX idx_exp_class   ON experiments(experiment_class);")
  ...
  # ── Genes ───────────────────────────────────────────────────────────────────
  DBI::dbAppendTable(con, "genes", data.frame(
    gene_id         = c("CC_0001",         "CC_0002",         "CC_0003",         "CC_0004",         "CC_0005"),
    cc_tag          = c("CCNA_00090",       "CCNA_00446",       "CCNA_02647",       "CCNA_02761",       "CCNA_01248"),
    gene_name       = c("ctrA",            "dnaA",            "fliF",            "pilA",            "rpoD"),
    ncbi_protein_id = c("WP_010919523.1",  "WP_010919456.1",  "WP_010921928.1",  "WP_010922042.1",  "WP_010920629.1"),
  ...
  # ── Experiments ─────────────────────────────────────────────────────────────
  DBI::dbAppendTable(con, "experiments", data.frame(
    experiment_id       = c("TC_RNAseq_WT_CC",                     "DE_ctrA_depl"),
    display_label       = c("WT cell cycle timecourse (RNA-seq)",   "ctrA depletion vs WT (RNA-seq)"),
  ...
  DBI::dbAppendTable(con, "experiment_conditions", data.frame(
  ...
  # ── DE results ──────────────────────────────────────────────────────────────
  DBI::dbAppendTable(con, "de_results", data.frame(
    gene_id       = c("CC_0001", "CC_0002", "CC_0003", "CC_0004", "CC_0005"),
  ...
  expr_patterns <- list(
    CC_0001 = c(5.2, 6.8, 7.1, 5.9, 4.8), # ctrA  – peaks in early S-phase
    CC_0002 = c(6.1, 5.8, 5.5, 6.2, 6.4), # dnaA  – relatively constant
    CC_0003 = c(4.5, 5.1, 6.3, 7.2, 6.8), # fliF  – late-cell-cycle induction
    CC_0004 = c(3.8, 4.2, 5.6, 6.9, 7.4), # pilA  – monotonically increasing
    CC_0005 = c(7.0, 6.9, 7.1, 6.8, 7.0)  # rpoD  – stable housekeeping
  )
  cond_labels <- c("t0", "t30", "t60", "t90", "t120")

  tc_expr <- do.call(rbind, lapply(names(expr_patterns), function(g) {
    data.frame(
      gene_id          = g,
